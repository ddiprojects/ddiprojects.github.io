...
*/
	$( '.dk-speakout-submit' ).click( function( e ) {
		...
		//test for required address fields if set
		if (document.getElementById('dk-speakout-street-' + id) && street === "") {
            $( '#dk-speakout-street-'  + id ).addClass( 'dk-speakout-error' ); 
			errors ++;
		}

		if (document.getElementById('dk-speakout-suburb-' + id) && suburb === "") {
            $( '#dk-speakout-suburb-'  + id ).addClass( 'dk-speakout-error' ); 
			errors ++;
		}
        if (document.getElementById('dk-speakout-city-' + id) && city === "") {
            $( '#dk-speakout-city-'  + id ).addClass( 'dk-speakout-error' ); 
			errors ++;
		}
		if (document.getElementById('dk-speakout-state-' + id) && state === "") {
            $( '#dk-speakout-state-'  + id ).addClass( 'dk-speakout-error' ); 
			errors ++;
		}
		if (document.getElementById('dk-speakout-postcode-' + id) && postcode === "") {
            $( '#dk-speakout-postcode-'  + id ).addClass( 'dk-speakout-error' ); 
			errors ++;
		}
		if (document.getElementById('dk-speakout-country-' + id) && country === "") {
		...
		if ( errors === 0 && $( this ).attr( 'rel' ) !== 'disabled' ) {
			...
			$.post( dk_speakout_js.ajaxurl, data,
				function( response ) {
					...
					// if you would like the petion form to vanish slowly after it is signed, uncomment the line above by removing the //
					// and comment the line below by adding // at the beginning.  You will have to do this any time this page is upgraded (rarely).
